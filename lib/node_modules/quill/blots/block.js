...
Block.allowedChildren = [_break.default, inline.default, parchment.EmbedBlot, text.default];
class BlockEmbed extends parchment.EmbedBlot {
  attach() {
    super.attach();
    this.attributes = new parchment.AttributorStore(this.domNode);
  }
  delta() {
    return new Delta.default().insert(this.value(), {
      ...this.formats(),
      ...this.attributes.values()
    });
  }
  format(name, value) {
    const attribute = this.scroll.query(name, parchment.Scope.BLOCK_ATTRIBUTE);
    if (attribute != null) {
...
function blockDelta(blot) {
  ...
  return blot.descendants(parchment.LeafBlot).reduce((delta, leaf) => {
    if (leaf.length() === 0) {
      return delta;
    }
    return delta.insert(leaf.value(), bubbleFormats(leaf, {}, filter));
  }, new Delta.default()).insert('\n', bubbleFormats(blot));
}
