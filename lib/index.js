...
const RichTextEditor = (props) => {
        ...
        if (Object.keys(modules).length > 0) {
            ...
            if (imageDrop) {
                quillModules.current.imageDrop =
                    imageDrop === false
                        ? imageDrop
                        : Object.assign({ imageHandler }, (typeof imageDrop === 'object' ? imageDrop : null));
            }
            // 默认支持自动识别URL
    ...
    useEffect(() => {
        ...
        if (title) {
            document
                ...
                .parentNode.insertBefore(document.querySelector(`#editor${editorId.current} + input`), document.getElementById(`editor${editorId.current}`));
        }
    }, []);
    useEffect(() => {
    ...
    const renderTitle = () => {
        if (title) {
            const config = title === true ? {} : title;
            return (React.createElement("input", { type: "text", placeholder: config.placeholder, className: "title-input", onChange: (e) => config.onChange(e.target.value), defaultValue: config.defaultValue }));
        }
        else {
            return null;
        }
