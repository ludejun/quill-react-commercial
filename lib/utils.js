...
// 超链接tooltip的保存操作
function saveLink(quill, startIndex) {
    var url = document.getElementById('link-url').value;
    if (url && isUrl(url)) {
        var words = document.getElementById('link-words').value || url;
        quill.insertText(startIndex, words, 'link', index.default(url, { stripWWW: false }), 'user');
        if (quill.theme)
...
exports.isUrl = isUrl;
exports.optionDisableToggle = optionDisableToggle;
