...
export function imageUpload(quill, imgUploadApi, uploadSuccCB, uploadFailCB) {
    ...
    fileInput.addEventListener('change', () => {
        ...
        // 请求图片保存API
        uploadImg(files[0], imgUploadApi, (url) => {
            modal.classList.add('ql-hidden');
            quill.enable(true);
            quill.editor.insertEmbed(range.index, 'image', url);
            quill.setSelection(range.index + 1, Quill.sources.SILENT);
    ...
    if (value === true) {
        ...
        if (selection === null || selection.length === 0) {
            ...
            document.querySelector('a.ql-action').onclick = () => {
                this.quill.insertText(selection.index, document.getElementById('link-words').value, 'link', document.getElementById('link-url').value, 'user');
