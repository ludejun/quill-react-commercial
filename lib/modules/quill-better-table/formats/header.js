...
class Header extends Block {
  static create (value) {
    ...
    table.CELL_IDENTITY_KEYS.forEach(key => {
      if (value[key]) node.setAttribute(`data-${key}`, value[key]);
    });

    table.CELL_ATTRIBUTES.forEach(key => {
      if (value[key]) node.setAttribute(`data-${key}`, value[key]);
    });
  ...
  static formats(domNode) {
    ...
    return table.CELL_ATTRIBUTES.concat(table.CELL_IDENTITY_KEYS).reduce((formats, attribute) => {
  ...
  format (name, value) {
    ...
    if (name === Header.blotName) {
      if (value) {
        ...
        if (row) {
          this.replaceWith(table.TableCellLine.blotName, {
            row,
            cell,
            rowspan,
            colspan
  ...
  optimize(context) {
    ...
    if (
      row &&
      !(this.parent instanceof table.TableCell)
    ) {
      this.wrap(table.TableCell.blotName, {
        row,
        colspan,
        rowspan
      });
