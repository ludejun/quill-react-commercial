...
const Break = quill.default.import('blots/break');
const Block = quill.default.import('blots/block');
const Container = quill.default.import('blots/container');
...
class TableContainer extends Container {
  ...
  deleteRow(compareRect, editorWrapper) {
    ...
    const removedRowsLength = this.rows().reduce((sum, row) => {
      let rowRect = index.getRelativeRect(row.domNode.getBoundingClientRect(), editorWrapper);
    ...
    fallCells.forEach((cell) => {
      ...
      const refCell = cellsInNextRow.reduce((ref, compareCell) => {
        const compareRect = index.getRelativeRect(
  ...
  tableDestroy() {
    const quill$1 = quill.default.find(this.scroll.domNode.parentNode);
    const tableModule = quill$1.getModule('better-table');
    this.remove();
    tableModule.hideTableTools();
    quill$1.update(quill.default.sources.USER);
  }
      ...
      if (cellRowspan > 1) {
        ...
        while (i > 1) {
          let refInNextRow = nextRow.children.reduce((result, cell) => {
            let compareRect = index.getRelativeRect(
              tableCell.domNode.getBoundingClientRect(),
              editorWrapper,
            );
            let cellRect = index.getRelativeRect(cell.domNode.getBoundingClientRect(), editorWrapper);
...
class TableViewWrapper extends Container {
  constructor(scroll, domNode) {
    super(scroll, domNode);
    const quill$1 = quill.default.find(scroll.domNode.parentNode);
...
TableCell.allowedChildren = [TableCellLine, header.default];
