...
var tableColumnTool = require('./modules/table-column-tool.js');
var tableSelection = require('./modules/table-selection.js');
var tableOperationMenu = require('./modules/table-operation-menu.js');
var nodeMatchers = require('./utils/node-matchers.js');
...
class BetterTable extends Module {
  static register() {
    quill.default.register(table.TableCol, true);
    quill.default.register(table.TableColGroup, true);
    quill.default.register(table.TableCellLine, true);
    quill.default.register(table.TableCell, true);
    quill.default.register(table.TableRow, true);
    quill.default.register(table.TableBody, true);
    quill.default.register(table.TableContainer, true);
    quill.default.register(table.TableViewWrapper, true);
    quill.default.register(table.TableViewWrapper, true);
  ...
  constructor(quill, options) {
    ...
    this.quill.root.addEventListener('click', (evt) => {
      ...
      const path = index.getEventComposedPath(evt);
  ...
  getTable(range = this.quill.getSelection()) {
    ...
    if (cellLine == null || cellLine.statics.blotName !== table.TableCellLine.blotName) {
  ...
  insertTable(rows, columns) {
    ...
    delta = new Array(rows).fill(0).reduce(memo => {
      let tableRowId = table.rowId();
      return new Array(columns).fill('\n').reduce((memo, text) => {
        memo.insert(text, { 'table-cell-line': {row: tableRowId, cell: table.cellId()} });
        return memo
      }, memo)
    }, delta);

    this.quill.updateContents(delta, quill.default.sources.USER);
    this.quill.setSelection(range.index + columns + 1, quill.default.sources.API);
  }

  showTableTools (table, quill, options) {
    this.table = table;
    this.columnTool = new tableColumnTool.default(table, quill, options);
    this.tableSelection = new tableSelection.default(table, quill, options);
  }
...
BetterTable.keyboardBindings = {
  ...
  'table-cell-line enter': {
    ...
    handler(range, context) {
      ...
      const Scope = quill.default.imports.parchment.Scope;
      if (range.length > 0) {
      ...
      this.quill.insertText(range.index, '\n', lineFormats['table-cell-line'], quill.default.sources.USER);
      ...
      Object.keys(context.format).forEach(name => {
        ...
        if (name === 'link') return;
        this.quill.format(name, context.format[name], quill.default.sources.USER);
      });
    },
  },
  ...
  return {
    ...
    handler(range, context) {
      ...
      if (targetRow != null && targetRow.statics.blotName === 'table-row') {
        ...
        this.quill.setSelection(index, 0, quill.default.sources.USER);
      } else {
...
function isTableCell (blot) {
  return blot.statics.blotName === table.TableCell.blotName
}
