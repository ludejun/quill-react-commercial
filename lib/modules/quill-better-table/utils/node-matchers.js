  ...
  delta = delta.reduce((newDelta, op) => {
    if (op.insert && typeof op.insert === 'string') {
      ...
      lines.forEach(text => {
        text === '\n'
        ? newDelta.insert('\n', op.attributes)
        : newDelta.insert(text, index._omit(op.attributes, ['table', 'table-cell-line']));
      });
    } else {
  ...
  return delta.reduce((newDelta, op) => {
    if (op.insert && typeof op.insert === 'string' &&
      ...
      newDelta.insert(op.insert, Object.assign(
        ...
        index._omit(op.attributes, ['table'])
      ));
    } else {
      // bugfix: remove background attr from the delta of table cell
      //         to prevent unexcepted background attr append.
      if (op.attributes && op.attributes.background && op.attributes.background === index.convertToHex(cellBg)) {
        newDelta.insert(op.insert, Object.assign(
          {},
          index._omit(op.attributes, ['table', 'table-cell-line', 'background'])
        ));
      } else {
        newDelta.insert(op.insert, Object.assign(
          {},
          index._omit(op.attributes, ['table', 'table-cell-line'])
        ));
      }
    }
