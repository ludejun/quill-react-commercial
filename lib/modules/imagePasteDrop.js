...
export class ImageDrop {
    ...
    constructor(quill, options = {}) {
        ...
        this.quill.on('text-change', (delta, oldDelta, source) => {
            if (this.imageHandler.imgUploadApi) {
                setTimeout(() => {
                    const imgs = quill.container.querySelectorAll('img[src^="data:"]:not(.uploading):not(.upload-fail)');
    ...
    b64ToUrl(base64) {
        return new Promise((resolve, reject) => {
            ...
            uploadImg(blob, imgUploadApi, (url) => {
                resolve(url);
            }, (error) => {
                reject(error);
            });
    ...
    showFailTooltip(id) {
        ...
        if (img) {
            ...
            if (tooltip) {
                tooltip.style.cssText = `${tooltip.style.cssText}left:${rect.left - containerRect.left - 1 + parent.scrollLeft}px;top:${rect.top - containerRect.top + parent.scrollTop}px;height:${img.height}px;width:${img.width}px;display: block;`;
                setTimeout(() => {
