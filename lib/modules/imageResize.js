...
var alignLeft = require('../node_modules/quill/assets/icons/align-left.svg.js');
var alignCenter = require('../node_modules/quill/assets/icons/align-center.svg.js');
var alignRight = require('../node_modules/quill/assets/icons/align-right.svg.js');
var _delete = require('../assets/icons/delete.svg.js');
var words = require('../assets/icons/words.svg.js');
...
const Parchment = quill.default.imports.parchment;
...
class Toolbar extends BaseModule {
  ...
  _defineAlignments = () => {
    const index = this.quill.getIndex(quill.default.find(this.img)); // the index of image
    this.alignments = [
      {
      ...
      },
      {
        ...
        apply: () => {
          ...
          const imgRemarkPre =
            this.options.imgRemarkPre || i18n.getI18nText('imgRemarkPre', this.options.i18n);
          this.quill.insertText(index + 1, `\n${imgRemarkPre}`, { color: '#999999', size: '12px' });
          this.quill.insertText(index + 2 + imgRemarkPre.length, '\n', { align });
          this.quill.setSelection(index + 2 + imgRemarkPre.length, quill.default.sources.SILENT);
      ...
      },
      {
        ...
        apply: () => {
          quill.default.find(this.img).deleteAt(0);
          this.hide();
        },
        isApplied: () => false,
      },
  ...
  _addToolbarButtons = () => {
    const buttons = [];
    const words = i18n.getI18nText(['alignLeft', 'alignCenter', 'alignRight', 'imgRemarkLabel', 'deleteImg'], this.options.i18n);
    this.alignments.forEach((alignment, idx) => {
      const button = document.createElement('span');
      buttons.push(button);
      button.innerHTML = iconsConfig.genIconDom(alignment.icon, words[idx]);
...
class ImageResize {
  ...
  showOverlay = () => {
    ...
    // 编辑器滚动隐藏 overlay
    this.quill.root.addEventListener('scroll', utils.throttle(() => {
  ...
  checkImage = (evt) => {
    if (this.img) {
      if (evt.keyCode === 46 || evt.keyCode === 8) {
        quill.default.find(this.img).deleteAt(0); // find为静态方法，原来这里有 bug 未生效
      }
      this.hide();
    }
  };
