    ...
    tableDialogOpen() {
        ...
        if (tableDialog) {
            ...
            document.getElementById('table-dialog-submit').addEventListener('click', () => {
                this.createTable(document.getElementById('row-number').value, document.getElementById('column-number').value);
            });
        }
    }
    showTableDialog() {
        ...
        const dialogContent = `<div class="table-dialog-content">${new Array(Number(dialogRows) > 0 ? Number(dialogRows) : 3)
            .fill(1)
            .map((x, row) => `<div class="table-dialog-tr">${new Array(Number(dialogColumns) > 0 ? Number(dialogColumns) : 4)
            .fill(2)
            .map((y, column) => `<div class="table-dialog-item" data-row="${row}" data-column="${column}"></div>`)
            .join('')}</div>`)
            .join('')}
      </div><div><p>
