...
class ToolbarTable extends Module {
  ...
  tableDialogOpen() {
    ...
    if (tableDialog) {
      ...
      Array.from(document.getElementsByClassName('table-dialog-item')).forEach(
        dom => {
  ...
  showTableDialog() {
    ...
    const dialogContent = `<div class="table-dialog-content">${
      new Array(Number(dialogRows) > 0 ? Number(dialogRows) : 3).fill(1).map(
        (x, row) => `<div class="table-dialog-tr">${
          new Array(Number(dialogColumns) > 0 ? Number(dialogColumns) : 4).fill(2).map(
            (y, column) => `<div class="table-dialog-item" data-row="${row}" data-column="${column}"></div>`,
          ).join('')}</div>`,
      ).join('')}
      </div><div><p>
    ...
    tableDialog.style = `top: ${tableIcon.offsetHeight +
      7}px; left: ${tableIcon.offsetLeft + 4}px;`;
